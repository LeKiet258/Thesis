...
def build(args):
    ...
    ## CIM
    sa = hooks.append(tb.sa.register_forward_hook(getActivation(f'sa')))
    ca = hooks.append(tb.ca.register_forward_hook(getActivation(f'ca')))
    ## SFA
...
def viz_fm(args):
    ...
    if not os.path.exists(f"./feature_maps/{par}"):
        os.makedirs(f"./feature_maps/{par}")
    else: # nếu tồn tại đường dẫn trên thì xoá đi tạo mới
        dir = f"./feature_maps/{par}"
        import shutil
        shutil.rmtree(dir) # remove
        os.makedirs(dir) # create new
    
    # viz sa
    sa_weight = activation['sa'][0].reshape(88,88,1)
    fig, axes = plt.subplots()
    axes.imshow(sa_weight[:, :, 0].cpu(), cmap='gray')
    axes.axis("off")
    plt.savefig(f"./feature_maps/{par}/sa.png")
    ...
    plt.savefig(f"./feature_maps/{par}/ca.png")
    ...
    for i, (fm, weight) in enumerate(activation.items()):
        ...
        for j in range(64):
            ...
            axes[j].axis("off")
        plt.savefig(f"./feature_maps/{par}/{fm}.png")
